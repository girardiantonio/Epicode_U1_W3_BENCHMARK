...
let exString = "I Love Coding";
let arrayString = splitMe(exString);
...
let exString2 = "Salve";
console.log(deleteOne(exString2, true));
...
let exString3 = "I have 4 dogs";
console.log(onlyLetters(exString3));
...
console.log(isThisAnEmail("antoniogirardi@gmail.com"));
...
function isTodayMyBirthday(birthday) {
  let today = new Date();
  let isSameDay = today.getDate() === birthday.getDate();
  let isSameMonth = today.getMonth() === birthday.getMonth();
  return isSameDay && isSameMonth;
}

let birthday = new Date(1999, 5, 9);
...
let updatedPerson = deleteProp(persona, "age");
...
function newestMovie(movies) {
  let newest = movies[0]; // Assume che il primo film sia il più recente

  for (let i = 1; i < movies.length; i++) {
    let currentMovie = movies[i];
    let currentYear = parseInt(currentMovie.Year); // Converte l'anno in un numero

    if (currentYear > parseInt(newest.Year)) {
      newest = currentMovie;
    }
  }
...
function searchByTitle(keyword, movies) {
  let matchingMovies = movies.filter(function (movie) {
...
function printTdText() {
  ...
  for (let i = 0; i < tdList.length; i++) {
    let tdText = tdList[i].textContent;
    console.log(tdText);
  }
}
...
function addNewElementToList() {
  ...
  newListElement.textContent = "Nuovo elemento";
